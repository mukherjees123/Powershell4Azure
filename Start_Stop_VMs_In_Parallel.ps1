...
function stopvms_frominputfile
{
$vmlistFile = "$($PSScriptRoot)\vmlist.txt"  
# Loading list of all VM objects in current subscription
($rmvms=Get-AzurermVM) > 0
#declaring an empty array to hold the VM objects that we want to start/stop
$vmrunninglist = @()
#reading the simple text file carrying the list of VMs
$vmlist = Get-Content $vmlistFile
#Filling the empty array $vmrunninglist with VM Objects. The idea is to pick each item in that text file(VM name), compare that to
#the existing $rmvms list and once match is found the corresponding VM OBject gets added in the array.  
...
#Action the start/stop from array members
